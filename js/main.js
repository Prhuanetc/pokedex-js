...
const allTypes = [
    "steel", "water", "dragon", "electric", "fairy", "ghost", "fire",
...
function getTipo(name) {
    ...
    return todosTipos[index] ? todosTipos[index].nome : "Desconhecido";
}

function getCor(name) {
    const index = allTypes.indexOf(name);
    return todosTipos[index] ? todosTipos[index].cor : "rgba(0, 0, 0, 0.75)";
}
